...
BIN_PATH=$WS_DIR/install/openarm_teleop/lib/openarm_teleop/bilateral_control
# ================================
# Check workspace
if [ ! -d "$WS_DIR" ]; then
...
ROS_ARGS=()
if [ -n "${OPENARM_BILATERAL_ROS_ARGS:-}" ]; then
    # Example:
    # export OPENARM_BILATERAL_ROS_ARGS="-p tesollo_force_to_torque_gain:=-0.06 -p tesollo_joint_effort_to_torque_gain:=-0.03"
    read -r -a ROS_ARGS <<< "$OPENARM_BILATERAL_ROS_ARGS"
    echo "[INFO] Extra ROS args: ${ROS_ARGS[*]}"
fi
