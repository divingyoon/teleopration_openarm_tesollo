...
// Publishes follower joint states (position, velocity, effort) on a
// side-specific topic: /openarm/left/joint_states or /openarm/right/joint_states
...
public:
    AdminThread(std::shared_ptr<RobotSystemState> leader_state,
        ...
        : PeriodicTimerThread(hz),
          ...
          arm_side_(arm_side)
    {
        prefix_ = (arm_side == "left_arm") ? "left" : "right";
        std::string topic = "/openarm/" + prefix_ + "/joint_states";
        joint_pub_ = ros_node_->create_publisher<sensor_msgs::msg::JointState>(topic, 10);
        std::cout << ">>> [ROS 2] Publisher created on topic: " << topic << std::endl;

        const std::string gripper_topic = "/openarm/" + prefix_ + "/leader/gripper_state";
        leader_gripper_pub_ = ros_node_->create_publisher<sensor_msgs::msg::JointState>(
            gripper_topic, 10);
        std::cout << ">>> [ROS 2] Publisher created on topic: "
                  << gripper_topic << std::endl;
    }
...
protected:
    ...
    void on_timer() override {
        ...
        for (size_t i = 0; i < follower_arm_resp.size(); ++i) {
            msg.name.push_back(prefix_ + "_follower_arm_joint_" + std::to_string(i));
        ...
        for (size_t i = 0; i < follower_hand_resp.size(); ++i) {
            msg.name.push_back(prefix_ + "_follower_hand_joint_" + std::to_string(i));
        ...
        if (leader_gripper_pub_ && !leader_hand_resp.empty()) {
            ...
            gripper_msg.header.stamp = ros_node_->now();
        ...
        if (pub_count++ % 500 == 0) {
            std::cout << ">>> [ROS 2] Publishing /openarm/" << prefix_
                      << "/joint_states... (" << pub_count << " times)" << std::endl;
        }
        ...
        if (csv_file_.is_open()) {
            if (first_tick_) {
                csv_file_ << "time_sec";
                for (size_t i = 0; i < follower_arm_resp.size(); ++i)
                    csv_file_ << ",arm_pos_" << i << ",arm_vel_" << i << ",arm_eff_" << i;
                for (size_t i = 0; i < follower_hand_resp.size(); ++i)
                    csv_file_ << ",hand_pos_" << i << ",hand_vel_" << i << ",hand_eff_" << i;
                csv_file_ << "\n";
                first_tick_ = false;
            }
        ...
        // follower follows leader, leader references follower position
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::cout << "=== OpenArm Unilateral Control v2 ===" << std::endl;
