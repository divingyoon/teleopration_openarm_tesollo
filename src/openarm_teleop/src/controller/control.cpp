...
void Control::SetEnableGripperReflectedEffort(bool enable) {
    enable_gripper_reflected_effort_ = enable;
}

void Control::SetBilateralGravityScale(double scale) {
    bilateral_gravity_scale_ = std::max(0.0, scale);
}

void Control::SetBilateralGravityScaleVector(const std::vector<double>& scale_vec) {
    bilateral_gravity_scale_vec_ = scale_vec;
}

void Control::SetBilateralGravityBiasVector(const std::vector<double>& bias_vec) {
    bilateral_gravity_bias_vec_ = bias_vec;
}
...
bool Control::bilateral_step() {
    ...
    for (size_t i = 0; i < arm_dof; i++) {
        ...
        if (i < bilateral_gravity_scale_vec_.size()) {
            axis_scale = std::max(0.0, bilateral_gravity_scale_vec_[i]);
        }
        double axis_bias = 0.0;
        if (i < bilateral_gravity_bias_vec_.size()) {
            axis_bias = bilateral_gravity_bias_vec_[i];
        }
        joint_arm_states_ref[i].effort =
            gravity[i] * bilateral_gravity_scale_ * axis_scale + friction[i] + axis_bias;
    }

    for (size_t i = 0; i < gripper_dof; i++) {
        const double reflected_effort =
            enable_gripper_reflected_effort_ ? joint_gripper_states_ref[i].effort : 0.0;
        joint_gripper_states_ref[i].effort = friction[i + arm_dof] + reflected_effort;
    }
